...
fun AppNavigation(
    ...
    NavHost(navController = navController, startDestination = Routes.HOME) {
        ...
        composable(Routes.HOME) {
            MainScreen(
                viewModel         = viewModel,
                onGoToLesson      = { lesson -> viewModel.setCurrentLesson(lesson); navController.navigate(Routes.LESSON) },
        ...
        composable(Routes.ADD_GOAL) {
            AddGoalScreen(
                ...
                onGoalAdded = { navController.navigate(Routes.GOALS) { popUpTo(Routes.ADD_GOAL) { inclusive = true } } }
            )
        }
